...
class Player {
    ...
    take(item) {
        if (this.location.pickups.map(i => i.toLowerCase()).includes(item.toLowerCase())) {
...
function createPlace(name,description,pickups=null,north="wall",east="wall",south="wall",west="wall") {
...
let garden = createPlace(
    ...
    pickups = ["glowing rose", "luminescent grass", "golden tulip", "singing bird", "whispering Tree"]
);
...
let livingRoom = createPlace(
    ...
    pickups = ["golden glasses", "velvet cushion", "ornate key", "silk ball", "antique vase","red elixir"]
);
...
let kitchen = createPlace(
    ...
    pickups = ["sourdough loaf", "aromatic herbs", "ancient recipe book", "freshly brewed potion", "crystal cup","blue elixir"]
);
...
let library = createPlace(
    ...
    pickups = ["ancient tome", "celestial map", "magical lens", "scrolls", "enchanted quill"]
);
...
function gamePlay() {
    ...
    if (playerName !== null && (playerName.length < 5 || /^\d+$/.test(playerName))) {
        alert("Player name cannot be less than 5 characters nor a number. Press again for automatic name");
        playerName="Vincent";
    } 
    ...
    document.getElementById("dispLocal").innerHTML=`${player1.location.toStr()}`;
}
...
function displayInventory() {
    ...
    if (showInventoryFlag==1) {
        document.getElementById("showinventory").innerHTML=player1.showInventory();
